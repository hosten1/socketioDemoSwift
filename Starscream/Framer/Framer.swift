    ...
    private func process() -> ProcessEvent {
        ...
        if opcode == .connectionClose {
            if payloadLen == 1 {
                ...
                closeCode = pointer.lym_readUint16(offset: offset)
                offset += size
        ...
        if payloadLen == 127 {
            ...
            dataLength = pointer.lym_readUint64(offset: offset)
            offset += size
            ...
            dataLength = UInt64(pointer.lym_readUint16(offset: offset))
            offset += size
        }
        ...
        if readDataLength == 0 {
            ...
            if isServer {
                payload = pointer.lym_unmaskData(maskStart: maskStart, offset: offset, length: readDataLength)
            } else {
...
public extension Array where Element: MyWSArrayType & UnsignedInteger {
     ...
     */
    func lym_readUint16(offset: Int) -> UInt16 {
     ...
     */
    func lym_readUint64(offset: Int) -> UInt64 {
        var value = UInt64(0)
        for i in 0...7 {
    ...
    func lym_unmaskData(maskStart: Int, offset: Int, length: Int) -> Data {
